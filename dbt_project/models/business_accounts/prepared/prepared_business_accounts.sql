...
prepared_accounts as (
    select
        ...
        coalesce(at.total_number_of_transactions, 0) as total_number_of_transactions,
        {{ calculate_total_transactions('at.total_transaction_volume') }} as total_transaction_volume,
        at.first_transaction_date,
        at.last_transaction_date
    from {{ ref('integrated_accounts') }} ia
    left join account_transactions at on ia.account_id = at.account_id
),
